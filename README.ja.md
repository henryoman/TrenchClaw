...
TrenchClaw は、自由形式のチャットボットではなく、制約付きのローカル実行ランタイムです。

短く正確に言うと、次のような構造です。

### 1) ランタイムが権威を持つ

- `apps/trenchclaw` がソースオブトゥルースです。
- `apps/frontends/gui` はランタイムのクライアントです。
- `apps/runner` はランタイムと GUI をまとめて起動する配布用ランチャーです。

### 2) ツールシステム

- ランタイムのツール定義は `apps/trenchclaw/src/tools` にあります。
- `src/tools/registry.ts` がランタイムアクションツールとワークスペースツールを定義します。
- `src/tools/snapshot.ts` が設定、ファイルシステムポリシー、リリース状態から現在のツールスナップショットを組み立てます。
- `src/ai/gateway/lanePolicy.ts` が各レーンに渡すツール集合を選びます。
- `src/runtime/chat/service.ts` がその選ばれたツールだけをモデルに登録します。

### 3) 実行境界

- ツールは実行前に型とスキーマで検証されます。
- 設定依存のポリシーチェックにより、危険な操作は拒否または確認必須にできます。
- ファイルシステムアクセスはランタイムのマニフェストとインスタンス境界で制限されます。
- `workspaceListDirectory`、`workspaceReadFile`、`workspaceBash` のようなワークスペース系の逃げ道も、ポリシーが許可している場合にだけ使えます。

### 4) 状態モデル

- `.runtime` はリポジトリで追跡される契約とテンプレート領域です。
- `.runtime-state` は実際の可変状態のルートです。
- アクティブインスタンスごとに、設定、Vault、ウォレット、ログ、キュー状態、ワークスペースを個別に持ちます。

### 5) なぜこれが重要か

この設計により、モデルは狭く明示的なツール面の上で動きつつ、必要なときにはランタイムワークスペースを調べたり、制約付きシェルを使ったりできます。目指しているのは、広いホスト権限を持つ曖昧なアシスタントではなく、状態遷移を追跡できるローカルのオペレーターランタイムです。
