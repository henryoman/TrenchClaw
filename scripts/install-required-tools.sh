...
print_manual_helius_install_help() {
  info "Helius CLI currently needs Node.js on PATH. Install Node.js first, then rerun this helper or install Helius CLI manually:"
...
install_or_update_helius() {
  ...
  if ! need_cmd node; then
    info "Skipping Helius CLI install/update because Node.js is unavailable."
    print_manual_helius_install_help
    return 0
  fi
