<script lang="ts">
  type SidebarTab = "chat" | "keys" | "settings" | "tracker" | "wakeup" | "info" | "wallets" | "schedule";
  type SidebarProps = {
...
<aside class="sidebar">
  ...
  <nav class="tabs" aria-label="Workspace tabs">
    ...
    <button
      type="button"
      class="tab-button {activeTab === 'tracker' ? 'active' : ''}"
      onclick={() => {
        onTabChange("tracker");
      }}>Tracker</button
    >
    <button
      type="button"
      class="tab-button {activeTab === 'wakeup' ? 'active' : ''}"
      onclick={() => {
        onTabChange("wakeup");
      }}>Wakeup</button
    >
