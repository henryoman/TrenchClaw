...
import { resolveWalletLibraryFilePath, walletGroupNameSchema } from "../wallet-based/create-wallets/wallet-storage";
...
const main = async (): Promise<void> => {
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    throw new Error(`Invalid timeout "${parsed.values["timeout-ms"]}"`);
  }
  ...
  const lamports = resolveLamports(amountSol);
