...
export const upsertSecret = async (
  ...
  context.addActivity("runtime", `Updated secret ${option.label}`);

  return {
    filePath: target.vaultPath,
    savedAt: new Date().toISOString(),
    entry: toSecretEntry(vaultData, option),
  };
};
...
export const deleteSecret = async (
  ...
  if (payload.optionId === SOLANA_RPC_OPTION_ID) {
    setByPath(vaultData, ["rpc", "default", "source"], "custom");
    setByPath(vaultData, ["rpc", "default", "public-id"], "");
    setByPath(vaultData, ["rpc", "default", "provider-id"], inferRpcProviderId(vaultData));
  }
  ...
  context.addActivity("runtime", `Deleted secret ${option.label}`);
