...
const RUNTIME_ACTION_RELEASE_READINESS_BY_NAME: Record<string, RuntimeReleaseReadinessDescriptor> = {
  ...
  getExternalWalletAnalysis: SHIPPED_NOW("External wallet portfolio analysis now ships through one compact read with holdings, recent swaps, and live SOL/USD context."),
  getExternalWalletHoldings: SHIPPED_NOW("External wallet holdings now ship through one compact read optimized for wallet scans and portfolio snapshots."),
  getRpcBalance: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcAccountInfo: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcMultipleAccounts: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcTokenAccountsByOwner: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcTokenSupply: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcTokenLargestAccounts: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcSignaturesForAddress: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  getRpcTransaction: SHIPPED_NOW("Raw Solana RPC read primitives now ship as compact JSON actions for model and operator use."),
  ...
  getTokenBiggestHolders: SHIPPED_NOW("Largest-holder reads now ship as one compact top-holders routine for exact token mints."),
  getTokenHolderDistribution: SHIPPED_NOW("On-chain token holder concentration reads ship in the current release."),
  getTokenRecentBuyers: SHIPPED_NOW("Recent token-buyer reads now ship as one compact recent-swap-window routine for exact token mints."),
...
const runtimeActionToolDefinitionsBase: readonly RuntimeActionToolDefinitionWithoutReadiness[] = [
  ...
  },
  {
    ...
    description: "Fetch the most recent Solana swaps for a wallet using Helius enhanced transaction history.",
    purpose: "Answer direct 'show me the last swaps for this wallet' requests through one compact recent-swaps routine with sensible defaults and display-ready timestamps.",
  ...
  {
    kind: "action",
    action: getTokenBiggestHoldersAction,
    description: "Return the biggest owner wallets for one SPL mint in one compact holder routine.",
    purpose: "Answer direct requests like 'show the top holders' or 'who are the biggest holders' without making the model combine raw largest accounts with owner aggregation on its own.",
    routingHint: "the user asks for the biggest holders or top owner wallets for one exact token mint",
    tags: ["rpc", "holders", "owners", "whales", "read"],
    exampleInput: {
      mintAddress: "DezXAZ8z7PnrnRJjz3wXBoRgixCa6f4t5D7N9m3bjsz",
      limit: 10,
      whaleThresholdPercent: 1,
  ...
  {
    kind: "action",
    action: getTokenRecentBuyersAction,
    description: "Fetch recent buyers for one token mint from a recent swap-output window.",
    purpose: "Inspect which wallets most recently bought a token, with explicit recent-window scope metadata and compact buy summaries.",
    routingHint: "the user asks for recent buyers or recent buy-side wallet activity for one exact token mint",
    tags: ["buyers", "token", "swaps", "market-data", "read"],
    exampleInput: {
  ...
  {
    kind: "action",
    action: getExternalWalletAnalysisAction,
    description: "Analyze one external wallet with current holdings, recent swaps, and a live SOL/USD snapshot.",
    purpose: "Answer external-wallet portfolio questions in one call when the user gives one exact wallet address and wants current holdings, recent trades, or rough USD value.",
    routingHint: "the user gave one exact external wallet address and wants holdings plus recent trades or valuation",
    tags: ["wallets", "holdings", "swaps", "portfolio", "read"],
    exampleInput: {
      walletAddress: "9xQeWvG816bUx9EPfK5Yw9s6o1tuVd7a3mZ9zNnV3xF",
      tradeLimit: 10,
    },
  ...
  {
    kind: "action",
    action: getExternalWalletHoldingsAction,
    description: "Fetch current holdings for one external wallet with SOL/USD valuation but without recent trade history.",
    purpose: "Answer external-wallet holdings, SOL balance plus USD value, and scan-list questions with one smaller payload when recent swaps are not needed.",
    routingHint: "the user gave one exact external wallet address and only wants current holdings, SOL balance plus USD value, or portfolio composition",
    tags: ["wallets", "holdings", "portfolio", "read"],
    exampleInput: {
      walletAddress: "9xQeWvG816bUx9EPfK5Yw9s6o1tuVd7a3mZ9zNnV3xF",
    },
    includeInCatalog: () => true,
  ...
  {
    kind: "action",
    action: getRpcBalanceAction,
    description: "Fetch the native SOL balance for one address through raw RPC.",
    purpose: "Expose the smallest raw SOL balance read when the model already knows one exact address and only needs lamports or SOL, not wallet holdings or USD valuation context.",
    routingHint: "the user gave one exact address and you only need its current SOL balance without wallet holdings or USD valuation context",
    tags: ["rpc", "balance", "sol", "read"],
    exampleInput: {
  ...
  {
    kind: "action",
    action: getRpcAccountInfoAction,
    description: "Fetch one account record through raw RPC with either base64 or parsed encoding.",
    purpose: "Inspect one exact on-chain account when you need owner, executable, lamports, parsed token info, or raw data without requesting broader wallet scans.",
    routingHint: "the user gave one exact account address and you need the account record itself",
    tags: ["rpc", "account", "read"],
    exampleInput: {
      account: "So11111111111111111111111111111111111111112",
      encoding: "jsonParsed",
    },
  ...
  {
    kind: "action",
    action: getRpcMultipleAccountsAction,
    description: "Fetch up to 200 exact account records through one batched raw RPC action.",
    purpose: "Batch exact account reads so the model can inspect a known small account set without wasting steps on repeated single-account calls.",
    routingHint: "you already know a concrete small set of account addresses and want one batch account read",
    tags: ["rpc", "accounts", "batch", "read"],
    exampleInput: {
      accounts: [
        "So11111111111111111111111111111111111111112",
        "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
  ...
  {
    kind: "action",
    action: getRpcTokenAccountsByOwnerAction,
    description: "Fetch token accounts for one owner filtered by mint or program id through raw RPC.",
    purpose: "Inspect one wallet's token-account rows directly when the model needs exact token accounts, not a higher-level holdings summary.",
    routingHint: "you need the exact token-account rows for one owner and one mint or token program",
    tags: ["rpc", "token-accounts", "wallets", "read"],
    exampleInput: {
      ownerAddress: "9xQeWvG816bUx9EPfK5Yw9s6o1tuVd7a3mZ9zNnV3xF",
  ...
  {
    kind: "action",
    action: getRpcTokenSupplyAction,
    description: "Fetch total supply metadata for one SPL token mint through raw RPC.",
    purpose: "Answer exact token supply and decimals questions without pulling broader market tooling or holder analysis when only supply metadata is needed.",
    routingHint: "you need the token supply and decimals for one exact mint",
    tags: ["rpc", "token", "supply", "read"],
    exampleInput: {
      mintAddress: "DezXAZ8z7PnrnRJjz3wXBoRgixCa6f4t5D7N9m3bjsz",
    },
    includeInCatalog: ({ settings }) => settings.trading.enabled,
  ...
  {
    kind: "action",
    action: getRpcTokenLargestAccountsAction,
    description: "Fetch the largest token accounts for one SPL mint through raw RPC.",
    purpose: "Inspect raw holder accounts for one mint before deeper aggregation or wallet-owner enrichment.",
    routingHint: "you need the largest token accounts for one exact mint before or instead of owner aggregation",
    tags: ["rpc", "holders", "largest-accounts", "read"],
    exampleInput: {
      mintAddress: "DezXAZ8z7PnrnRJjz3wXBoRgixCa6f4t5D7N9m3bjsz",
      limit: 10,
    },
  ...
  {
    kind: "action",
    action: getRpcSignaturesForAddressAction,
    description: "Fetch recent transaction signatures for one address through raw RPC.",
    purpose: "List a compact recent signature window when the model needs a lightweight activity cursor before deciding whether to fetch full parsed transactions.",
    routingHint: "you need recent signatures for one address before deeper transaction inspection",
    tags: ["rpc", "transactions", "signatures", "read"],
    exampleInput: {
      account: "9xQeWvG816bUx9EPfK5Yw9s6o1tuVd7a3mZ9zNnV3xF",
      limit: 10,
    },
    includeInCatalog: ({ settings }) => settings.trading.enabled,
    enabledBySettings: ({ settings }) => settings.trading.enabled,
  ...
  {
    kind: "action",
    action: getRpcTransactionAction,
    description: "Fetch one parsed transaction record through raw RPC by signature.",
    purpose: "Inspect one exact transaction after signature discovery without committing to a heavier wallet-history workflow.",
    routingHint: "you already know one exact signature and need that transaction record",
    tags: ["rpc", "transaction", "read"],
    exampleInput: {
      signature: "5m3YvQY7eR8k8cTtJmJ4M1m2n3p4q5r6s7t8u9v0w1x2y3z4",
      encoding: "jsonParsed",
    },
    includeInCatalog: ({ settings }) => settings.trading.enabled,
    enabledBySettings: ({ settings }) => settings.trading.enabled,
