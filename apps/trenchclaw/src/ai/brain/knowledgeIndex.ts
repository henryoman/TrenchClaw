...
import { CORE_APP_ROOT } from "../../runtime/runtimePaths";
...
const renderKnowledgePromptSummaryFromEntries = (entries: KnowledgeLookupEntry[]): string => {
  ...
  return [
    ...
    "- Workflow: if you already know the alias, call **`readKnowledgeDoc`** directly. Otherwise call **`listKnowledgeDocs`** with a short `query` (optional `tier`: `core`, `deep`, `support`, `skills`, or `all`) and then open the exact alias.",
  ...
  const summaryPromise = (async () => {
    try {
      ...
      return [
        ...
        "- Workflow: **`listKnowledgeDocs`** with a short `query` (optional `tier`: `core`, `deep`, `support`, `skills`, or `all`) → **`readKnowledgeDoc`** with the exact `doc` alias once you know it. If you already know the alias, skip listing and read directly.",
        '- Use `tier = "skills"` when the task matches a packaged skill workflow, not a one-off doc.',
        "",
