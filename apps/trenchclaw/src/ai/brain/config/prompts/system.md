...
- If no typed runtime action covers the needed read and a bounded trusted CLI command can answer it, use `workspaceBash` with a structured type (`cli`, `version`, `help`, `which`, `search_text`, `list_directory`, `http_get`) instead of raw shell (`shell`).
...
- When using `workspaceBash`, always prefer the structured types that provide explicit validation over raw shell commands.
