...
- Use `workspaceBash` only for real shell or CLI work; send a typed `params.type` as documented, preferring structured types (`cli`, `version`, `help`, `which`, `search_text`, `list_directory`, `http_get`) over raw shell (`shell`).
