...
# Dev-only helper: ensures latest Bun + Solana CLI + Helius CLI are installed.
...
install_or_upgrade_helius() {
  need_cmd node || fail "Node.js is required for the current Helius CLI install. Install Node.js, then rerun this helper."
  add_path_for_current_process "$BUN_BIN_DIR"
  ensure_path_in_shell_profiles "$BUN_BIN_DIR"
...
main() {
  ...
  install_or_upgrade_solana
  install_or_upgrade_helius
  info "Development tool bootstrap complete."
