...
var resolveAppRoot = () => {
  ...
  for (const candidate of candidates) {
    if (existsSync(path.join(candidate, "apps/trenchclaw/package.json")) && existsSync(path.join(candidate, "apps/frontends/gui"))) {
      return candidate;
    }
  }
...
var RUNNER_LOG_PREFIX = colorize("@trenchclaw:", "neonPurple");
...
var createBufferedLogRelay = (sink, maxBufferedLines = 500) => {
  const buffer = [];
...
var relaySubprocessOutput = async (stream, relay) => {
  if (!stream) {
    return;
  }
